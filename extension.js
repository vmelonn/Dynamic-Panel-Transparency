...
export default class DynamicPanelExtension extends Extension {
    enable() {
        ...
        try {
            ...
            console.error('Dynamic Panel: Error in enable():', error);
        }
    }
    ...
    disable() {
        ...
        try {
            ...
            console.error('Dynamic Panel: Error in disable():', error);
        }
    }
    ...
    _connectSignals() {
        try {
            ...
            console.error('Dynamic Panel: Error connecting signals:', error);
        }
    }
    ...
    _disconnectSignals() {
        this._signalIds.forEach(([object, id]) => {
            try {
                ...
                console.error('Dynamic Panel: Error disconnecting signal:', error);
            }
        });
        this._signalIds = [];
    ...
    _connectExistingWindowSignals() {
        try {
            ...
            console.error('Dynamic Panel: Error connecting existing window signals:', error);
        }
    }
    ...
    _connectWindowSignals(win) {
        ...
        try {
            ...
            console.error('Dynamic Panel: Error connecting window signals:', error);
        }
    }
        ...
        try {
            ...
            const normalWindows = windows.filter(win => {
                try {
                    ...
                    console.error('Dynamic Panel: Error filtering window:', e);
                    return false;
                }
            });

            // Filter for visible (non-minimized) normal windows
            const visibleWindows = normalWindows.filter(win => {
                try {
                    return !win.minimized;
                } catch (e) {
                    console.error('Dynamic Panel: Error checking minimized state:', e);
                    return false;
                }
            });
            ...
            const hasFullscreen = visibleWindows.some(win => {
                try {
                    ...
                    const maximizedOpaque = this._settings ? this._settings.get_boolean('maximized-opaque') : false;
                    ...
                    console.error('Dynamic Panel: Error checking fullscreen/maximized state:', e);
                    return false;
                }
            });
            ...
            console.error('Dynamic Panel: Error updating panel state:', error);
        }
    }
    ...
    _setPanelState(state, reason) {
        ...
        try {
            ...
            if (this._settings) {
                animationDuration = this._settings.get_int('animation-duration');
            } else {
                animationDuration = 300; // fallback
            }

            switch (state) {
                case 'transparent':
                    opacity = this._settings ? this._settings.get_int('transparent-opacity') / 100 : 0;
                    panelStyle = `background-color: rgba(45, 45, 45, ${opacity}) !important; transition: background-color ${animationDuration}ms ease;`;
                    break;

                case 'semi-opaque':
                    opacity = this._settings ? this._settings.get_int('semi-opaque-opacity') / 100 : 0.85;
                    panelStyle = `background-color: rgba(45, 45, 45, ${opacity}) !important; transition: background-color ${animationDuration}ms ease;`;
                    break;

                case 'opaque':
                default:
                    opacity = this._settings ? this._settings.get_int('opaque-opacity') / 100 : 1.0;
                    panelStyle = `background-color: rgba(45, 45, 45, ${opacity}) !important; transition: background-color ${animationDuration}ms ease;`;
                    break;
            }
            ...
            console.error('Dynamic Panel: Error setting panel state:', error);
        }
    }
}
